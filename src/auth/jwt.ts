...
const createAccessToken = (user: User, expiresIn: number) => {
...
const createRefreshToken = (user: User, expiresIn: number) => {
...
const renewAccessToken = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    console.log(error);
    return res.status(401).json({ error: true, message: "Unable to renew access token!" });
  }
};
...
const verifyToken = (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    console.log(error);
    return res.status(401).json({ error: true, message: "Oops! Something went wrong!" });
  }
};
